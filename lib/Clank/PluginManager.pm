...
#   * Perl modules:  <dir>/lib/Clank/Wit/<Name>.pm or <dir>/<file>.pm, with a
#     register($api) method (see wits.example/hello).
#   * Declarative decks (clank-old format): directories of .wit files — TOML
#     metadata + embedded Perl source — optionally grouped into subdirs and
#     described by a deck.toml manifest.  Loaded via Clank::Wit::Loader; each
...
#     ships Clank::Logic/Clank::Rules); lib/ is added to @INC before any .wit in
#     the deck compiles, since wit sources compile at load time.
package Clank::PluginManager;
use strict;
use warnings;
use Clank::Wit::API;      # direct dependency: load_dir constructs one per wit
use Clank::Wit::Dispatch;
...
sub new {
    ...
    $self->{dispatch} = Clank::Wit::Dispatch->new(wits => $self->{dwits});
    return $self;
}
...
sub _read_manifest {
    ...
    for my $mf (qw(deck.toml wit.toml)) {
        ...
        close $fh;
        require Clank::Wit::File;
        my $meta = eval { Clank::Wit::File::parse_toml($content) };
...
# PATH lookup for requires_bin — same rule as Clank::Wit::Loader::_have_bin.
sub _have_bin {
    my ($bin) = @_;
...
#   namespaces => [ 'Clank::Logic', ... ]  package prefixes the unit may ship
...
#   1. CLANK_WITS_PATH (colon list)   2. ./.clank/wits (project)
#   3. <clank home>/wits (user; CLANK_HOME or ~/.clank — Clank::Util::clam_home)
#   4. explicit -w/--wit paths
sub discover_roots {
    my ($self, %o) = @_;
    require Clank::Util;
    my @roots;
    push @roots, split /:/, $ENV{CLANK_WITS_PATH} if defined $ENV{CLANK_WITS_PATH} && length $ENV{CLANK_WITS_PATH};
    push @roots, '.clank/wits';
    push @roots, Clank::Util::clam_home() . '/wits';
...
sub _wit_dirs {
    ...
    # .wit files (e.g. `clank -w decks/logic`).  Deliberately NOT the one-level-
...
#   <dir>/lib/Clank/Wit/<Name>.pm    module wit (standard; first module wins)
...
sub load_dir {
    ...
    if (_has_wit($dir)) {
        ...
        require Clank::Wit::Loader;
        my $api = Clank::Wit::API->new(
            bus => $self->{bus}, store => $self->{store}, session => $self->{session},
            ui => $self->{ui}, wit_name => $name,
        );
        my @records = @{ Clank::Wit::Loader->load_dir($self, $api, $dir) };
        push @{ $self->{wits} }, { name => $name, pkg => 'Clank::Wit::File', dir => $dir, wit => undef, api => $api, meta => $meta, state => 'active' };
        $self->{apis}{$name} = $api;
    ...
    my ($pkg, $file);
    if (-d "$dir/lib") {
        # User wits live in Clank/Wits/, harness infrastructure in Clank/Wit/
        my $wits_dir = -d "$dir/lib/Clank/Wits" ? "$dir/lib/Clank/Wits" : "$dir/lib/Clank/Wit";
        my @mods;
        if (-d $wits_dir) {
        ...
        unless (@mods) {
            push @{ $self->{errors} }, "$dir: no lib/Clank/Wits/*.pm or lib/Clank/Wit/*.pm";
            warn "[wits] $dir: no wit modules found, skipping\n";
            return;
        }
        (my $mod = shift @mods) =~ s{\.pm$}{};
        ($pkg, $file) = ("Clank::Wits::$mod", "$wits_dir/$mod.pm");
        # If module is in Clank/Wit/ (harness infrastructure), use that namespace
        if ($wits_dir =~ m{/Clank/Wit$}) {
            ($pkg, $file) = ("Clank::Wit::$mod", "$wits_dir/$mod.pm");
        }
        ...
        ($file, $pkg) = ("$dir/$pms[0]", _pkg_from_file("$dir/$pms[0]") // "Clank::Wit::$name");
...
sub load_builtins {
    ...
    for my $module (@modules) {
        eval {
            # require with a variable and Clank::Wit::File loaded triggers a
...
sub enable_wit {
    ...
    if ($rec->{pkg} eq 'Clank::Wit::File') {
        require Clank::Wit::Loader;
        my @records = @{ Clank::Wit::Loader->load_dir($self, $api, $rec->{dir}) };
