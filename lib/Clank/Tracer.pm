# Clank::Tracer — event-trace log for pipeline observability.
#
...
#
# All spans are persisted in SQLite via Clank::Store for post-hoc query.
package Clank::Tracer;
use strict;
use warnings;
use Clank::Util qw(uuid4 jencode jdecode);
use Time::HiRes qw(clock_gettime CLOCK_MONOTONIC);

sub new {
    my ($class, %args) = @_;
    my $store = $args{store} // die "Clank::Tracer requires store\n";
    my $self = bless {
        store     => $store,
        bus       => $args{bus},
...
Clank::Tracer — event-trace log for pipeline observability. Records who
...
=head1 SYNOPSIS
  ...
  my $t = Clank::Tracer->new(store => $store);
  ...
  my $t = Clank::Tracer->new(store => $store, bus => $bus, auto_subscribe => 1);

=head1 DESCRIPTION

Clank::Tracer makes the LLM -> rules -> world model -> crystallization
...
L<Clank::Bus>, L<Clank::Store>, L<Clank::Governor>
