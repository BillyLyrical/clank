# Clank::EventSourcing — immutable state-change log for "why did it do that?"
...
sub new {
    my ($class, %args) = @_;
    my $store = $args{store} // die "Clank::EventSourcing requires store\n";
    my $self = bless {
        store    => $store,
        bus      => $args{bus},
...
Clank::EventSourcing is an append-only event log. Every state change —
...
L<Clank::WorldModel>, L<Clank::Rules::Engine>, L<Clank::Tracer>
