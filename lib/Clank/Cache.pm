# Clank::Cache — TTL cache for LLM responses and world model queries.
#
...
#
# Storage: SQLite via Clank::Store. Entries expire after TTL.
...
sub new {
    my ($class, %args) = @_;
    my $store = $args{store} // die "Clank::Cache requires store\n";
    my $self = bless {
        store     => $store,
        bus       => $args{bus},
...
=head1 SYNOPSIS
  ...
  my $wm_cache = Clank::Cache->new(store => $store, namespace => 'worldmodel');
...
Clank::Cache is a TTL cache backed by SQLite. It stores arbitrary Perl
...
L<Clank::Provider>, L<Clank::Store>, L<Clank::Governor>
