# Clank::Governor — rate limiter, budget cap, and circuit breaker for LLM
...
#
# State is persisted in SQLite via Clank::Store. Rate-limit windows are
...
sub new {
    my ($class, %args) = @_;
    my $store = $args{store} // die "Clank::Governor requires store\n";
    my $self = bless {
        store    => $store,
        bus      => $args{bus},
...
=head1 SYNOPSIS
  ...
  my $gov = Clank::Governor->new(
      store          => $store,          # required: Clank::Store
      bus            => $bus,            # optional: Clank::Bus for events
...
Clank::Governor is the Watt's centrifugal governor for LLM providers. It
...
State is persisted in SQLite via Clank::Store (C<governor_usage> table).
...
L<Clank::Provider>, L<Clank::Loop>, L<Clank::Store>
