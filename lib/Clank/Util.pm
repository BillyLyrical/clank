...
# The clank home directory: state, workdirs, installed wits, index, lockfile.
# CLANK_HOME overrides; defaults to ~/.clank (or '.' when HOME is unset).
sub clam_home {
    return $ENV{CLANK_HOME} // (defined $ENV{HOME} ? "$ENV{HOME}/.clank" : '.');
}
