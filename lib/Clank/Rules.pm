# Clank::Rules — rule engine facade: parse DSL, build engines over a store.
# The fact store is shared SQLite state (Clank::Store) — every agent on the same
...
# Parse a rules-DSL program -> arrayref of Clank::Rules::Rule.  (method call)
sub parse {
    my ($class, $text) = @_;
    return Clank::Rules::DSL->parse($text);
}

# Build an engine over a store, optionally pre-loaded with DSL rules:
#   Clank::Rules->engine(store => $store, strategy => 'first', dsl => <<'D')
sub engine {
    my ($class, %args) = @_;
    my $engine = Clank::Rules::Engine->new(%args);
    if (defined $args{dsl}) {
        $engine->load(Clank::Rules::DSL->parse($args{dsl}));
    }
    return $engine;
}
...
Clank::Rules - Rule engine facade with forward/backward chaining, negation,
...
=head1 SYNOPSIS
  ...
  my $store = Clank::Store->new(db => 'clank.db');
...
Clank::Rules is the rule engine facade. It parses DSL rules, builds engines
over a shared SQLite fact store (Clank::Store), and provides multiple inference
...
Parse a rules-DSL program into an arrayref of Clank::Rules::Rule configs.
...
L<Clank::Rules::Engine>, L<Clank::Rules::Rule>, L<Clank::Rules::DSL>,
L<Clank::Store>
