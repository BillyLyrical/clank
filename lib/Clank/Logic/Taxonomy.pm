# Clank::Logic::Taxonomy — hierarchical classification for entities, beliefs, and goals.
#
...
sub new {
    my ($class, %args) = @_;
    my $world_model = $args{world_model} or die "Clank::Logic::Taxonomy requires world_model\n";
...
=head1 SYNOPSIS
  ...
  use Clank::Logic::Taxonomy;

  my $tx = Clank::Logic::Taxonomy->new(
      world_model => $wm,             # required: Clank::WorldModel
      bus         => $bus,            # optional
...
Clank::Logic::Taxonomy provides hierarchical classification across the world model.
