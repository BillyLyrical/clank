# Clank::Wit::File — declarative wit files (.wit), ported from clank-old.
#
...
#   PERL
#
# The handler contract (unchanged from clank-old): the compiled closure is
...
sub parse_file {
    ...
    # clank-old Loader: line-based, terminator must sit alone on its line).
...
sub parse_toml {
    ...
    my @a = @_;
    shift @a if @a && !ref $a[0] && $a[0] =~ /^Clank::Wit::File\b/;
    my ($text, %opts) = @a;
...
# ---------------------------------------------------------------------------
# Clank::Wit::File::Record — the lightweight $self passed to handlers.
...
# Same compilation as clank-old: the source is the body of a closure that
