# Clank::Wit::Loader — loads declarative .wit files (clank-old format) into a
...
sub load_dir {
    ...
    for my $file (sort @files) {
        ...
        # the clank-old Loader: deduction/axiom.wit -> "deduction.axiom" (path
...
sub _load_one {
    ...
    my $parsed = Clank::Wit::File->parse_file($file);   # dies on malformed files
    ...
    my $code = Clank::Wit::File->compile($source, name => $name);   # dies on error
    ...
    $rec = {
        ...
        meta  => $meta,
        code  => $code,
        wit   => bless({ %$meta }, 'Clank::Wit::File::Record'),
    ...
    if ($bus) {
        for my $topic (@{ $meta->{subscribes} // [] }) {
            my $id = $bus->subscribe($topic, sub {
                ...
                return if $Clank::Wit::Loader::BUS_AGENT_DEPTH >= 5;
                local $Clank::Wit::Loader::BUS_AGENT_DEPTH = $Clank::Wit::Loader::BUS_AGENT_DEPTH + 1;
...
sub _execute {
    ...
    my %ctx = (
        wits      => $pm->{dispatch},
        bus       => $pm->{bus} ? Clank::Wit::Loader::BusAdapter->new($pm->{bus}) : undef,
        store     => $pm->{store},
        session   => $pm->{session},
        config    => {},
...
# pending SIGALRM kill the process — see Clank::Tools::Bash for the history).
sub _with_timeout {
    my ($secs, $code) = @_;
...
# ---------------------------------------------------------------------------
# Clank::Wit::Loader::BusAdapter — the object wits receive as $ctx{bus}.
...
# ---------------------------------------------------------------------------
package Clank::Wit::Loader::BusAdapter;
