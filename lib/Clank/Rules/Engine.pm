# Clank::Rules::Engine — pluggable inference over rules. Strategies: first,
...
#
# Facts live in the shared SQLite store (Clank::Store) — the blackboard every
...
package Clank::Rules::Engine;
use strict;
use warnings;
use Clank::Rules::Rule ();   # load() constructs these

sub new {
    my ($class, %args) = @_;
    die "Clank::Rules::Engine requires a store (Clank::Store)" unless $args{store};
    my $self = bless {
        store           => $args{store},
...
# Load rules from an arrayref of rule configs (hashrefs for Clank::Rules::Rule).
sub load {
    my ($self, $rule_configs) = @_;
    for my $cfg (@$rule_configs) {
        $self->add(Clank::Rules::Rule->new(%$cfg));
    }
}
