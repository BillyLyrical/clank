# Clank::Driver — programmatic multi-query session driver for the clank harness.
#
# The API wrapper an AI harness (or any Perl script) uses to drive a complex,
# stateful Clank session: many prompts against one persistent session, with
# structured per-turn results and full bus-event observability.  See
# docs/DRIVER.md for the design notes and the clankd NDJSON protocol built on
# top of this module.
#
#   my $d = Clank::Driver->new(provider => 'lmstudio', model => '...', db => ':memory:');
#   $d->start;
...
sub ask {
    ...
    my $before = scalar @{ Clank::Session::Messages::chain($store, $sid) };
    ...
    my %r = (
        ...
        response       => $response,
        tools          => \@tools,
        messages_added => scalar @{ Clank::Session::Messages::chain($store, $sid) } - $before,
    );
...
sub messages {
    my ($self, %o) = @_;
    die "Clank::Driver: not started\n" unless $self->{session};
    my @chain = @{ Clank::Session::Messages::chain($self->store, $self->session_id) };
...
sub events {
    ...
    if (defined $o{topic}) {
        require Clank::Bus;
        @rows = grep { Clank::Bus::topic_matches($o{topic}, $_->{topic} // '') } @rows;
...
# Compact session summary (used by clankd's session_info command).
sub info {
    my ($self) = @_;
    die "Clank::Driver: not started\n" unless $self->{session};
    ...
    return {
        ...
        messages     => scalar @{ Clank::Session::Messages::chain($self->store, $self->session_id) },
