# Clam::Director — multi-turn orchestration: plan, goal, force-tool.
#
...
sub new {
    my ($class, %args) = @_;
    my $loop = $args{loop} or die "Clam::Director requires loop\n";
    ...
    if ($self->{world_model}) {
        $self->{planner} = Clam::Logic::GoalPlanner->new(
