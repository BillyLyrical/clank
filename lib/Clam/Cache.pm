# Clam::Cache — TTL cache for LLM responses and world model queries.
#
...
#
# Storage: SQLite via Clam::Store. Entries expire after TTL.
...
sub new {
    my ($class, %args) = @_;
    my $store = $args{store} // die "Clam::Cache requires store\n";
    my $self = bless {
        store     => $store,
        bus       => $args{bus},
...
=head1 SYNOPSIS
  ...
  my $wm_cache = Clam::Cache->new(store => $store, namespace => 'worldmodel');
...
Clam::Cache is a TTL cache backed by SQLite. It stores arbitrary Perl
...
L<Clam::Provider>, L<Clam::Store>, L<Clam::Governor>
