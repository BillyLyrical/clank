# Clam::EventSourcing — immutable state-change log for "why did it do that?"
...
sub new {
    my ($class, %args) = @_;
    my $store = $args{store} // die "Clam::EventSourcing requires store\n";
    my $self = bless {
        store    => $store,
        bus      => $args{bus},
...
Clam::EventSourcing is an append-only event log. Every state change —
...
L<Clam::WorldModel>, L<Clam::Rules::Engine>, L<Clam::Tracer>
