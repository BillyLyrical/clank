package AI::Clam::Provider;
use strict; use warnings;
# Base class for LLM providers (OpenAI-compatible chat completions).
# Config precedence: CLI flags > env (CLAM_PROVIDER/CLAM_MODEL/CLAM_BASE_URL/CLAM_API_KEY)
#   > ~/.clam/config.json > defaults. API keys are never written to DB or logs.

sub new {
    my ($class, %o) = @_;
    my $cfg_file = "$ENV{HOME}/.clam/config.json";
    my %file_cfg;
    if (-f $cfg_file) {
        require JSON::PP;
    ...
    my %c = (
        name      => $o{name}       // $ENV{CLAM_PROVIDER} // $file_cfg{provider} // 'lmstudio',
        model     => $o{model}      // $ENV{CLAM_MODEL}    // $file_cfg{model}      // 'local-model',
        base_url  => $o{base_url}   // $ENV{CLAM_BASE_URL} // $file_cfg{base_url}   // 'http://localhost:1234/v1',
        api_key   => $o{api_key}    // $ENV{CLAM_API_KEY}  // $file_cfg{api_key},
