# AI::Clam::Wit::Loader — loads declarative .wit files (clam-old format) into a
...
package AI::Clam::Wit::Loader;
use strict;
use warnings;
use File::Find;
use AI::Clam::Wit::File;
...
sub _load_one {
    ...
    my $parsed = AI::Clam::Wit::File->parse_file($file);   # dies on malformed files
    ...
    my $code = AI::Clam::Wit::File->compile($source, name => $name);   # dies on error
    ...
    $rec = {
        ...
        meta  => $meta,
        code  => $code,
        wit   => bless({ %$meta }, 'AI::Clam::Wit::File::Record'),
    ...
    if ($bus) {
        for my $topic (@{ $meta->{subscribes} // [] }) {
            my $id = $bus->subscribe($topic, sub {
                ...
                return if $AI::Clam::Wit::Loader::BUS_AGENT_DEPTH >= 5;
                local $AI::Clam::Wit::Loader::BUS_AGENT_DEPTH = $AI::Clam::Wit::Loader::BUS_AGENT_DEPTH + 1;
...
sub _execute {
    ...
    my %ctx = (
        wits      => $pm->{dispatch},
        bus       => $pm->{bus} ? AI::Clam::Wit::Loader::BusAdapter->new($pm->{bus}) : undef,
        store     => $pm->{store},
        session   => $pm->{session},
        config    => {},
...
# pending SIGALRM kill the process — see AI::Clam::Tools::Bash for the history).
sub _with_timeout {
    my ($secs, $code) = @_;
...
# ---------------------------------------------------------------------------
# AI::Clam::Wit::Loader::BusAdapter — the object wits receive as $ctx{bus}.
...
# ---------------------------------------------------------------------------
package AI::Clam::Wit::Loader::BusAdapter;
