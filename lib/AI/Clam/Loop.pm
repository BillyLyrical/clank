...
sub new {
    my ($class, %o) = @_;
    my $session = $o{session} or die "AI::Clam::Loop requires session\n";
    return bless {
        session   => $session,
        stream    => $o{stream} // 0,
...
sub run_prompt {
    ...
    while (!$self->{aborted}) {
        ...
        if ($self->{max_tools} && @all_tools > $self->{max_tools}) {
            require AI::Clam::ToolSelector;
            my $last_msg = $msgs->[-1]{content} // '';
            my $selected = AI::Clam::ToolSelector->select(
                tools  => \@all_tools,
                prompt => $last_msg,
                max    => $self->{max_tools},
            );
...
sub spawn {
    ...
    require AI::Clam::Session;
    my $child_session = AI::Clam::Session->new(
        store    => $store,
        bus      => $bus,
