# AI::Clam::Metrics — simple counters for LLM calls, tokens, rules, crystallizations.
#
...
sub new {
    my ($class, %args) = @_;
    my $store = $args{store} // die "AI::Clam::Metrics requires store\n";
    my $self = bless {
        store       => $store,
        bus         => $args{bus},
