# AI::Clam::Director — multi-turn orchestration: plan, goal, force-tool.
#
...
sub new {
    my ($class, %args) = @_;
    my $loop = $args{loop} or die "AI::Clam::Director requires loop\n";
    ...
    if ($self->{world_model}) {
        $self->{planner} = AI::Clam::Logic::GoalPlanner->new(
