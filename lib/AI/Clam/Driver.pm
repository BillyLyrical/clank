# AI::Clam::Driver — programmatic multi-query session driver for the clam harness.
#
...
# top of this module.
#
#   my $d = AI::Clam::Driver->new(provider => 'lmstudio', model => '...', db => ':memory:');
#   $d->start;
...
sub ask {
    my ($self, $text, %o) = @_;
    die "AI::Clam::Driver: not started\n" unless $self->{session};
    ...
    my $before = scalar @{ AI::Clam::Session::Messages::chain($store, $sid) };
    ...
    my %r = (
        ...
        response       => $response,
        tools          => \@tools,
        messages_added => scalar @{ AI::Clam::Session::Messages::chain($store, $sid) } - $before,
    );
...
sub messages {
    my ($self, %o) = @_;
    die "AI::Clam::Driver: not started\n" unless $self->{session};
    my @chain = @{ AI::Clam::Session::Messages::chain($self->store, $self->session_id) };
...
sub events {
    ...
    if (defined $o{topic}) {
        require AI::Clam::Bus;
        @rows = grep { AI::Clam::Bus::topic_matches($o{topic}, $_->{topic} // '') } @rows;
...
sub info {
    ...
    return {
        ...
        messages     => scalar @{ AI::Clam::Session::Messages::chain($self->store, $self->session_id) },
