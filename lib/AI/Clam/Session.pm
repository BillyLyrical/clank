...
package AI::Clam::Session;
use strict;
use warnings;
use Cwd qw(getcwd);
use AI::Clam::Session::Messages ();
use AI::Clam::Session::SystemPrompt ();
...
sub new {
    my ($class, %o) = @_;
    my $store = $o{store} or die "AI::Clam::Session requires store";
...
sub system_prompt {
    ...
    $self->{system_prompt} = AI::Clam::Session::SystemPrompt::build(
        cwd            => $self->{cwd},
...
sub build_context {
    my ($self) = @_;
    my $chain = AI::Clam::Session::Messages::chain($self->{store}, $self->{id});
    return AI::Clam::Session::Messages::to_provider_list($chain);
}

sub add_user_message      { my ($s, $t)  = @_; AI::Clam::Session::Messages::add($s->{store}, $s->{id}, role => 'user', content => $t) }
sub add_assistant_message { my ($s, %a)  = @_; AI::Clam::Session::Messages::add($s->{store}, $s->{id}, role => 'assistant', content => \%a) }
sub add_tool_result       {
    my ($s, $tc_id, $out, $is_err) = @_;
    AI::Clam::Session::Messages::add($s->{store}, $s->{id}, role => 'toolResult',
        content => { tool_call_id => $tc_id, output => $out, isError => $is_err ? 1 : 0 });
}

sub est_context_tokens {
    my ($self) = @_;
    return AI::Clam::Session::Messages::est_tokens([
