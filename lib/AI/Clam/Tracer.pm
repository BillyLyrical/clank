# AI::Clam::Tracer — event-trace log for pipeline observability.
#
...
#
# All spans are persisted in SQLite via AI::Clam::Store for post-hoc query.
package AI::Clam::Tracer;
use strict;
use warnings;
...
sub new {
    my ($class, %args) = @_;
    my $store = $args{store} // die "AI::Clam::Tracer requires store\n";
    my $self = bless {
        store     => $store,
        bus       => $args{bus},
...
AI::Clam::Tracer — event-trace log for pipeline observability. Records who
...
=head1 SYNOPSIS
  ...
  my $t = AI::Clam::Tracer->new(store => $store, bus => $bus, auto_subscribe => 1);

=head1 DESCRIPTION

AI::Clam::Tracer makes the LLM -> rules -> world model -> crystallization
...
L<AI::Clam::Bus>, L<AI::Clam::Store>, L<AI::Clam::Governor>
