# AI::Clam::Rules::Engine — pluggable inference over rules. Strategies: first,
...
#
# Facts live in the shared SQLite store (AI::Clam::Store) — the blackboard every
...
package AI::Clam::Rules::Engine;
use strict;
use warnings;
use AI::Clam::Rules::Rule ();   # load() constructs these

sub new {
    my ($class, %args) = @_;
    die "AI::Clam::Rules::Engine requires a store (AI::Clam::Store)" unless $args{store};
    my $self = bless {
        store           => $args{store},
...
# Load rules from an arrayref of rule configs (hashrefs for AI::Clam::Rules::Rule).
sub load {
    my ($self, $rule_configs) = @_;
    for my $cfg (@$rule_configs) {
        $self->add(AI::Clam::Rules::Rule->new(%$cfg));
    }
}
