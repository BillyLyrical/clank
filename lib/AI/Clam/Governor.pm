# AI::Clam::Governor — rate limiter, budget cap, and circuit breaker for LLM
...
#
# State is persisted in SQLite via AI::Clam::Store. Rate-limit windows are
...
sub new {
    my ($class, %args) = @_;
    my $store = $args{store} // die "AI::Clam::Governor requires store\n";
    my $self = bless {
        store    => $store,
        bus      => $args{bus},
...
=head1 SYNOPSIS
  ...
  my $gov = AI::Clam::Governor->new(
      store          => $store,          # required: AI::Clam::Store
      bus            => $bus,            # optional: AI::Clam::Bus for events
...
AI::Clam::Governor is the Watt's centrifugal governor for LLM providers. It
...
State is persisted in SQLite via AI::Clam::Store (C<governor_usage> table).
...
L<AI::Clam::Provider>, L<AI::Clam::Loop>, L<AI::Clam::Store>
