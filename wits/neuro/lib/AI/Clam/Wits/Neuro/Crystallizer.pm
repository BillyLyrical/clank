# CLAM-WIT: name=Crystallizer
# CLAM-WIT: version=1.0.0
# CLAM-WIT: about=Capture LLM solutions as deterministic rules — system gets cheaper with use
# CLAM-WIT: usage=Auto-crystallizes on agent_end bus event. Heuristic extraction by default; LLM extraction when provider available.
# CLAM-WIT: hint=crystallization, rules, LLM solutions, deterministic, learning
...
package AI::Clam::Wits::Neuro::Crystallizer;
use strict;
use warnings;
use AI::Clam::Util qw(now_ms jencode jdecode);
use AI::Clam::Rules::Rule ();
...
sub _register_rule {
    ...
    my $rule = AI::Clam::Rules::Rule->new(
        name     => $pattern->{name},
        type     => $pattern->{type},
