...
subtest 'Cosine similarity' => sub {
    require Clank::Wits::Embedding::Embedding;

    # Identical vectors = 1.0
    my $sim = Clank::Wits::Embedding::Embedding::_cosine_sim([1, 0, 0], [1, 0, 0]);
    is($sim, 1.0, 'identical vectors');

    # Orthogonal vectors = 0.0
    $sim = Clank::Wits::Embedding::Embedding::_cosine_sim([1, 0, 0], [0, 1, 0]);
    ok(abs($sim) < 1e-10, 'orthogonal vectors');

    # Opposite vectors = -1.0
    $sim = Clank::Wits::Embedding::Embedding::_cosine_sim([1, 0], [-1, 0]);
    ok(abs($sim + 1) < 1e-10, 'opposite vectors');

    # Similar vectors = high score
    $sim = Clank::Wits::Embedding::Embedding::_cosine_sim([1, 2, 3], [1, 2, 3.1]);
    ok($sim > 0.99, 'similar vectors score high');

    # Empty vectors = 0
    $sim = Clank::Wits::Embedding::Embedding::_cosine_sim([], []);
...
subtest 'Vector CSV round-trip' => sub {
    ...
    my $csv = Clank::Wits::Embedding::Embedding::_vec_to_csv($vec);
    is($csv, '0.1,0.2,-0.3,0.456789', 'CSV encoding');

    my $parsed = Clank::Wits::Embedding::Embedding::_parse_embedding($csv);
    is_deeply($parsed, $vec, 'CSV round-trip');

    is(Clank::Wits::Embedding::Embedding::_parse_embedding(undef), undef, 'undef input');
    is(Clank::Wits::Embedding::Embedding::_parse_embedding(''), undef, 'empty input');
};
...
subtest 'Provider detection' => sub {
    ...
    local $ENV{CLANK_OLLAMA_URL} = 'http://localhost:99999';   # non-existent
    local $ENV{CLANK_EMBEDDING_API} = undef;
    local $ENV{CLANK_EMBEDDING_KEY} = undef;

    my $p = Clank::Wits::Embedding::Embedding::_detect_provider();
...
subtest 'Entity embedding storage' => sub {
    ...
    for my $row (@$rows) {
        ...
        my $score = Clank::Wits::Embedding::Embedding::_cosine_sim($query_vec, $vec);
...
subtest 'Ollama embedding (live)' => sub {
    ...
    my $vec = eval { Clank::Wits::Embedding::Embedding::_embed_ollama($provider, 'hello world') };
    ...
    my $vec2 = Clank::Wits::Embedding::Embedding::_embed_ollama($provider, 'hello world');
    my $sim = Clank::Wits::Embedding::Embedding::_cosine_sim($vec, $vec2);
    ok($sim > 0.99, 'identical text scores near 1.0');

    my $vec3 = Clank::Wits::Embedding::Embedding::_embed_ollama($provider, 'quantum physics');
    my $sim2 = Clank::Wits::Embedding::Embedding::_cosine_sim($vec, $vec3);
