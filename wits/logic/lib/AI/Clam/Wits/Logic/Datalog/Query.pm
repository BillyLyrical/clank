...
# CLAM-WIT: about=Query a Datalog program: facts + Horn-clause rules with first-order unification and proof search
# CLAM-WIT: usage=Input: { program?: str, goal: str, facts?: [str] } Output: { ok: 1, solutions: [{Var: term, ...}] } or { ok: 0, error: str }
# CLAM-WIT: hint=datalog_query, datalog, query, horn_clause, unification, proof_search
# CLAM-WIT: author=CLAM
# CLAM-WIT: license=Artistic-2.0
package AI::Clam::Wits::Logic::Datalog::Query;
use strict;
use warnings;
...
sub register {
    ...
    $api->register_tool(
        ...
        execute => sub {
            ...
            my $kb = eval { AI::Clam::Logic->parse($program) };
            return { ok => 0, error => "program: $@" } if $@;

            $goal =~ s/\.\z// unless ref $goal;
            my $sols = eval { AI::Clam::Logic->query($kb, ref $goal ? $goal : "$goal") };
