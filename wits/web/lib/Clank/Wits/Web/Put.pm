...
# CLANK-WIT: about=HTTP PUT request with JSON or form data
# CLANK-WIT: usage=Input: { url: "https://api.example.com/data/1", content: "{\"key\":\"val\"}" } Output: { status: 200, content: "..." }
# CLANK-WIT: hint=http put request, update, api call, rest
# CLANK-WIT: author=Clank
# CLANK-WIT: license=Artistic-2.0
package Clank::Wits::Web::Put;
use strict;
use warnings;
...
sub register {
    ...
    $api->register_tool(
        name        => 'web_put',
        description => 'HTTP PUT request with JSON or form data',
        parameters  => {
            type       => 'object',
            properties => {
                url          => { type => 'string', description => 'URL to put to' },
