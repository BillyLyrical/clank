...
subtest 'Construction' => sub {
    my $c = Clam::Crystallizer->new(store => $store);
    isa_ok($c, 'Clam::Crystallizer');
    my $s = $c->stats;
...
subtest 'Crystallize from text' => sub {
    my $c = Clam::Crystallizer->new(store => Clam::Store->new(db => ':memory:'));
...
subtest 'Low confidence rules are rejected' => sub {
    my $c = Clam::Crystallizer->new(store => Clam::Store->new(db => ':memory:'), min_confidence => 0.9);
...
subtest 'Disabled crystallizer does nothing' => sub {
    my $c = Clam::Crystallizer->new(store => Clam::Store->new(db => ':memory:'), enabled => 0);
...
subtest 'Crystallized rules are registered in engine' => sub {
    ...
    require Clam::Rules;
    my $engine = Clam::Rules->engine(store => $store2);

    my $c = Clam::Crystallizer->new(store => $store2, engine => $engine);
    $c->crystallize(conversation => "Gold is a precious metal.");
...
subtest 'Metrics tracking' => sub {
    ...
    my $metrics = Clam::Metrics->new(store => $store4);

    my $c = Clam::Crystallizer->new(store => $store4, metrics => $metrics);
