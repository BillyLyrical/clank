# Live end-to-end smoke: drive clankd --stdio against a REAL model server and
# verify multi-query continuity through the full stack (driver -> app -> loop
# -> provider -> HTTP).  Skipped unless CLANK_LIVE_BASE_URL is set, so plain
# `prove t/` stays offline-safe.
#
#   CLANK_LIVE_BASE_URL=http://192.168.1.12:1234/v1 \
#   CLANK_LIVE_MODEL=qwen3.8-27b \
#   [CLANK_LIVE_PROVIDER=lmstudio] prove -l t/10_e2e_live.t
...
my $base_url = $ENV{CLANK_LIVE_BASE_URL};
plan skip_all => 'set CLANK_LIVE_BASE_URL (and optionally CLANK_LIVE_MODEL / CLANK_LIVE_PROVIDER) to run the live e2e smoke'
    unless defined $base_url && length $base_url;

my $model    = $ENV{CLANK_LIVE_MODEL} // 'qwen3.8-27b';
my $provider = $ENV{CLANK_LIVE_PROVIDER} // 'lmstudio';
...
# spawn clankd (IPC::Open3 arg order is write-to-child FIRST — see t/09)
my @cmd = ($^X, "$FindBin::RealBin/../bin/clankd", '--stdio',
    '--provider', $provider, '--base_url', $base_url, '--model', $model,
    '--db', "$tmp/live.db", '--timeout', '180');
my ($w, $r);
my $e = gensym;
my $pid = IPC::Open3::open3($w, $r, $e, @cmd) or die "spawn clankd: $!";

sub rpc {
    my ($req, $label, $max_secs) = @_;
    print {$w} jencode($req), "\n";
    local $SIG{ALRM} = sub { die "clankd did not answer '$label' within ${max_secs}s\n" };
    alarm($max_secs // 240);
    my $line = <$r>;
    alarm(0);
    die "no response from clankd ($label)" unless defined $line;
    chomp $line;
    return jdecode($line);
}
...
$resp = rpc({ id => 2, prompt => 'Reply with exactly one word: Clank' }, 'prompt-1', 300);
is($resp->{ok}, 1, 'live: first prompt ok');
like(($resp->{response} // ''), qr/Clank/i, 'live: model answered through full stack');
...
like(($resp->{response} // ''), qr/Clank/i, 'live: second turn remembered the first (context continuity)');
...
local $SIG{ALRM} = sub { die "clankd did not exit after shutdown\n" };
alarm(10);
