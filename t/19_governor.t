...
subtest 'Construction with defaults' => sub {
    my $gov = Clank::Governor->new(store => $store);
    isa_ok($gov, 'Clank::Governor');
...
subtest 'Rate limit - per minute' => sub {
    ...
    my $gov = Clank::Governor->new(store => $store2, max_per_minute => 3);
...
subtest 'Rate limit - per hour' => sub {
    ...
    my $gov = Clank::Governor->new(store => $store3, max_per_hour => 2);
...
subtest 'Rate limit - tokens per hour' => sub {
    ...
    my $gov = Clank::Governor->new(store => $store4, max_tokens_hour => 500);
...
subtest 'Circuit breaker trips after threshold' => sub {
    ...
    my $gov = Clank::Governor->new(store => $store6, cb_threshold => 3, cb_cooldown_ms => 100);
...
subtest 'Circuit breaker half-open after cooldown' => sub {
    ...
    my $gov = Clank::Governor->new(store => $store7, cb_threshold => 2, cb_cooldown_ms => 1);
...
subtest 'Circuit breaker closes on success from half-open' => sub {
    ...
    my $gov = Clank::Governor->new(store => $store8, cb_threshold => 1, cb_cooldown_ms => 1);
...
subtest 'Non-fatal failures do not trip circuit' => sub {
    ...
    my $gov = Clank::Governor->new(store => $store9, cb_threshold => 2);
...
subtest 'Cost calculation with known model' => sub {
    my $store10 = Clank::Store->new(db => ':memory:');
    my $gov = Clank::Governor->new(store => $store10, session_id => 'cost-test');
...
subtest 'Cost zero for unknown model' => sub {
    my $store11 = Clank::Store->new(db => ':memory:');
    my $gov = Clank::Governor->new(store => $store11);
...
subtest 'Custom pricing overrides defaults' => sub {
    my $store12 = Clank::Store->new(db => ':memory:');
    my $gov = Clank::Governor->new(
        store   => $store12,
...
subtest 'Usage tracking' => sub {
    my $store13 = Clank::Store->new(db => ':memory:');
    my $gov = Clank::Governor->new(store => $store13, budget => 10.00, session_id => 'usage-test');
...
subtest 'Manual circuit reset' => sub {
    my $store14 = Clank::Store->new(db => ':memory:');
    my $gov = Clank::Governor->new(store => $store14, cb_threshold => 1);
...
subtest 'Session cost persists in SQLite' => sub {
    my $store15 = Clank::Store->new(db => ':memory:');

    my $gov1 = Clank::Governor->new(store => $store15, session_id => 'persist-test');
    $gov1->record(model => 'gpt-4o', input_tokens => 1000, output_tokens => 500);

    my $gov2 = Clank::Governor->new(store => $store15, session_id => 'persist-test');
...
subtest 'Events published to bus' => sub {
    my $store16 = Clank::Store->new(db => ':memory:');
    require Clank::Bus;
    my $bus = Clank::Bus->new(store => $store16);
