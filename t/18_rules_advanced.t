...
subtest 'Backward Chaining - prove existing fact' => sub {
    my $engine = Clam::Rules->engine(store => $store, strategy => 'first');
...
subtest 'Backward Chaining - prove via rule' => sub {
    ...
    my $engine = Clam::Rules->engine(store => $store2, strategy => 'first');
    
    $engine->assert_fact('parent', { parent => 'Bob', child => 'Alice' });
    
    $engine->add(Clam::Rules::Rule->new(
...
subtest 'Negation as Failure - not_exists' => sub {
    ...
    my $engine = Clam::Rules->engine(store => $store3, strategy => 'first');
...
subtest 'Negation as Failure - in production rule' => sub {
    ...
    my $engine = Clam::Rules->engine(store => $store4, strategy => 'first');
...
subtest 'Conflict Resolution - priority wins' => sub {
    ...
    my $engine = Clam::Rules->engine(store => $store5, strategy => 'first');
...
subtest 'Conflict Resolution - random strategy' => sub {
    ...
    my $engine = Clam::Rules->engine(store => $store6, strategy => 'random');
    ...
    my %winners;
    for my $try (1..20) {
        my $store_try = Clam::Store->new(db => ':memory:');
        my $engine_try = Clam::Rules->engine(store => $store_try, strategy => 'random');
        $engine_try->assert_fact('trigger', { id => 1 });
        for my $i (1..3) {
            $engine_try->add(Clam::Rules::Rule->new(
                name       => "rule_$i",
                type       => 'production',
                priority   => $i,
                conditions => [{ type => 'trigger', id => 1 }],
...
subtest 'Rule Composition - pipeline' => sub {
    ...
    my $engine = Clam::Rules->engine(store => $store7, strategy => 'first');
...
subtest 'Incremental Re-evaluation' => sub {
    ...
    my $engine = Clam::Rules->engine(store => $store8, strategy => 'first');
...
subtest '_match_condition_with_negation' => sub {
    ...
    my $engine = Clam::Rules->engine(store => $store9, strategy => 'first');
