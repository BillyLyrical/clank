...
# ===========================================================================
# 8. clamd stdio front-end (child process)
# ===========================================================================
my @clamd_cmd = ($^X, "$FindBin::RealBin/../bin/clamd", '--stdio', '--provider', 'mock', '--db', ':memory:');
...
sub spawn_clamd {
    ...
    eval {
        ...
        $pid = IPC::Open3::open3($w, $r, $e, @clamd_cmd);
        1;
    } or die "open3 failed: $@";
...
sub rpc {
    ...
    print {$w} jencode($req), "\n";
    my $line = <$r>;
    die "no response from clamd" unless defined $line;
    chomp $line;
    return jdecode($line);
}
...
sub reap {
    my ($pid, $what) = @_;
    local $SIG{ALRM} = sub { die "clamd did not exit within 10s: $what\n" };
    alarm(10);
...
{
    my ($r, $w, $e, $pid) = spawn_clamd();

    my $resp = rpc($w, $r, { id => 1, command => 'ping' });
    is_deeply({ ok => $resp->{ok}, pong => $resp->{pong}, id => $resp->{id} },
              { ok => 1, pong => 1, id => 1 }, 'clamd ping');

    $resp = rpc($w, $r, { id => 2, prompt => 'hello clam' });
    is($resp->{ok}, 1, 'clamd prompt ok');
    like($resp->{response} // '', qr/mock: hello clam/, 'mock provider echoed through full stack');
    ...
    like(jencode($resp->{events}), qr/first|hello clam/, 'second turn context includes first (via events)');
    ...
    close $w; close $r;
    is(reap($pid, 'shutdown'), 0, 'clamd exits 0 after shutdown (no zombie)');
...
# EOF teardown: parent closes the pipe WITHOUT shutdown — child must still exit.
{
    ...
    close $w;   # stdin EOF — the old clam-sock leaked here
    close $r;
    is(reap($pid, 'stdin EOF'), 0, 'clamd exits 0 on stdin EOF (zombie regression)');
    close $e;
}
