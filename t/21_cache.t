...
subtest 'Construction with defaults' => sub {
    my $cache = Clam::Cache->new(store => $store);
    isa_ok($cache, 'Clam::Cache');
    my $s = $cache->stats;
...
subtest 'Set and get basic' => sub {
    my $c = Clam::Cache->new(store => Clam::Store->new(db => ':memory:'));
...
subtest 'make_key is deterministic' => sub {
    my $k1 = Clam::Cache->make_key(model => 'gpt-4o', messages => [{ role => 'user', content => 'hi' }]);
    my $k2 = Clam::Cache->make_key(model => 'gpt-4o', messages => [{ role => 'user', content => 'hi' }]);
    is($k1, $k2, 'same input produces same key');

    my $k3 = Clam::Cache->make_key(model => 'gpt-4o', messages => [{ role => 'user', content => 'bye' }]);
    ok($k1 ne $k3, 'different input produces different key');

    my $k4 = Clam::Cache->make_key(model => 'gpt-4o-mini', messages => [{ role => 'user', content => 'hi' }]);
...
subtest 'Entry expires after TTL' => sub {
    my $c = Clam::Cache->new(store => Clam::Store->new(db => ':memory:'), ttl_ms => 1);
...
subtest 'Namespaces are isolated' => sub {
    ...
    my $c_llm = Clam::Cache->new(store => $store2, namespace => 'llm');
    my $c_wm  = Clam::Cache->new(store => $store2, namespace => 'worldmodel');
...
subtest 'Eviction when max_entries exceeded' => sub {
    my $c = Clam::Cache->new(store => Clam::Store->new(db => ':memory:'), max_entries => 5);
...
subtest 'Cache publishes events to bus' => sub {
    ...
    my $c = Clam::Cache->new(store => $store3, bus => $bus);
    my @events;
